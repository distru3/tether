...
|  |  - Native Hosts & Domain Enforcer (`st-enforce-win`)                  |  |
|  |  - Cloudflare Family DNS Adapter Configurator (`st-dns`)                |  |
|  +-------------------------------------------------------------------------+  |
|                                   ▲                                           |
...
2. **Two-Tier Fail-Safe Suppression**:
   ...
   - **Tier 2 (Session Front Guard - `st-session::main::is_game_or_fullscreen`)**: Even before a newly discovered app is classified by the agent, the session front inspects the focused window snapshot (`FocusedSnapshot`) every 1 Hz tick:
     ...
     - **Accurate Borderless Fullscreen & Taskbar Geometry Guard (`is_window_rect_fullscreen`)**: Accurately distinguishes borderless fullscreen games from standard maximized desktop applications. If the window leaves the Windows taskbar/appbar uncovered (`rcWork` vs `rcMonitor`), or if it has standard maximized window chrome (`WS_MAXIMIZE` and `WS_CAPTION`), it is never classified as a fullscreen game. True borderless fullscreen windows covering the physical monitor without standard captions are accurately detected.
